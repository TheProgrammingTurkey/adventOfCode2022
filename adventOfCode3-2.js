...
for(let i = 0; i < elves.length; i+=3){
  const firstBag = elves[i].split("");
  const secondBag = elves[i+1].split("");
  const thirdBag = elves[i+2].split("");
  const duplicate = check(firstBag, secondBag, thirdBag)
  const letter = duplicate.charCodeAt(0)
...
function check(firstBag, secondBag, thirdBag){
  for(let i = 0; i < firstBag.length; i++){
    for(let j = 0; j < secondBag.length; j++){
      for(let k = 0; k < thirdBag.length; k++){
        if(firstBag[i] == secondBag[j] && secondBag[j] == thirdBag[k]){
        return firstBag[i]
        } 
      } 
    }
